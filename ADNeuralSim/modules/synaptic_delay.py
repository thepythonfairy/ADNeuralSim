...
'''
Instructions: Write a function that assigns synaptic delays to each connection in a connectvity matrix. It should return
...
Notes: this function will be called from main_simulation.ipynb using the parameters from paramters.py. We are dealing
with parameter abstraction so please do not code any actual values. Accept inputs and return the matrix.

Integration:
from synaptic_delay import generate_synaptic_delays
delays = generate_synaptic_delays(matrix)
