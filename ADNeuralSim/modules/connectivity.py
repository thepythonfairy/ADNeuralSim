...
def generate_connectivity_matrix(num_neurons: int, connection_prob: float)->np.ndarray:
    if num_neurons < 0:
        raise ValueError("num_neurons must be non-negative")
    if not (0 <= connection_prob <= 1):
        raise ValueError("connection_prob must be in [0, 1]")

    #Edge Case: zero neurons --> empty matrix
    if num_neurons == 0:
       return np.empty((0, 0), dtype=int)

    #Base random Bernoulli matrix
    mat = (np.random.rand(num_neurons, num_neurons) < connection_prob).astype(int)
    ...
    if connection_prob == 0:
       return np.zeros((num_neurons, num_neurons), dtype=int)
    if connection_prob == 1:
       mat [:] = 1
       np.fill_diagonal(mat, 0)
       return mat
